...
# SCPN MIF Plasma Jet Core — diagnostic and clock semantics
...
identities for the plasma-jet magneto-inertial configuration this
...
from scpn_mif_plasma_jet_core.configuration import ConsistencyFinding
from scpn_mif_plasma_jet_core.errors import DiagnosticPlanError
...
ASYMMETRY_BAND_HZ: Final = (1.0e04, 1.0e08)
ARRIVAL_TIMING_CEILING_S: Final = 1.0e-07
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-MIF-PLASMA-JET-CORE"
                )
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-MIF-PLASMA-JET-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented plasma-jet MIF diagnostic practice; empty
            ...
            if (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            "declared merge-asymmetry band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the jet merge and "
                            "compression asymmetry range "
                            "10 kHz-100 MHz (Hsu 2012)"
                        ),
                    )
                )
            if (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            " is coarser than the sub-us "
                            "jet-arrival spread of merging arrays "
                            "(Hsu 2012)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the jet merge-and-compression scale of "
                            "up to ~100 us (Hsu 2012)"
                        ),
                    )
                )
