...
# SCPN MIF Plasma Jet Core — device configuration container

"""Device configuration container bound to the SPO reactor registry.

A :class:`DeviceConfiguration` composes a validated jet array under the
single registry identifier this repository owns. The multi-jet
requirement is hard; an array below the documented spherically
convergent size is flagged (Hsu et al., IEEE TPS 40 (2012) 1287).
...
OWNED_CONFIGURATIONS: Final = ("plasma_jet_mif",)
SPHERICAL_ARRAY_MIN_JETS: Final = 30
...
class DeviceConfiguration:
    """Validated plasma-jet-MIF device configuration.
    ...
    def __post_init__(self) -> None:
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-MIF-PLASMA-JET-CORE; owned: {OWNED_CONFIGURATIONS!r}"
            )
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            the array reaches the documented spherically convergent
            size. Findings are advisory instruments, not machine
            claims.
        """
        findings: list[ConsistencyFinding] = []
        if self.jets.jet_count < SPHERICAL_ARRAY_MIN_JETS:
            findings.append(
                ConsistencyFinding(
                    field="jets.jet_count",
                    message=(
                        f"jet count {self.jets.jet_count} is below the "
                        f"documented spherically convergent array size "
                        f"{SPHERICAL_ARRAY_MIN_JETS}"
                    ),
                )
            )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "jets": {
                "jet_count": self.jets.jet_count,
                "jet_mass_mg": self.jets.jet_mass_mg,
                "jet_velocity_km_s": self.jets.jet_velocity_km_s,
            },
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "jets", "registry"}
    unknown = sorted(set(record) - known)
    if unknown:
        raise DeviceConfigurationError(f"record: unknown fields {unknown!r}")
    jets = _require_mapping(record, "jets")
    registry = _require_mapping(record, "registry")
    return DeviceConfiguration(
        identifier=_string(record, "identifier"),
        jets=JetArray(
            jet_count=_integer(jets, "jet_count"),
            jet_mass_mg=_number(jets, "jet_mass_mg"),
            jet_velocity_km_s=_number(jets, "jet_velocity_km_s"),
        ),
        registry=RegistryBinding(
