...
SCPN MIF Plasma Jet Core — CHANGELOG
-->
...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the convergent jet array used by the README, the
  array-size gate, and the launch-merge-implode sequence).
...
- Diagnostic and clock semantics model
  (`src/scpn_mif_plasma_jet_core/observability.py`), the second implemented
...
- Device configuration model (`src/scpn_mif_plasma_jet_core/`), the first implemented
