...
# SCPN MIF Plasma Jet Core — device configuration container tests
...
def test_owned_identifier_constructs() -> None:
    """The owned identifier constructs with a convergent array."""
    assert synthetic_configuration().identifier == "plasma_jet_mif"
...
def test_consistency_report_clean_and_finding() -> None:
    """The report is empty at array size and precise below it."""
    assert synthetic_configuration().consistency_report() == ()
    sparse = synthetic_configuration(jet_count=6)
    findings = sparse.consistency_report()
    assert len(findings) == 1
    assert "spherically convergent" in findings[0].message
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: {**r, "jets": None}, "jets: must be an object"),
...
def test_from_record_field_type_violations() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["jets"]["jet_count"] = 2.5
    with pytest.raises(DeviceConfigurationError, match="jet_count: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
    record["jets"]["jet_count"] = True
    with pytest.raises(DeviceConfigurationError, match="jet_count: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
    record["jets"]["jet_mass_mg"] = "big"
    with pytest.raises(DeviceConfigurationError, match="jet_mass_mg: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
...
def test_integer_accepted_where_number_expected() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["jets"]["jet_velocity_km_s"] = 50
    restored = configuration_from_record(record)
    assert restored.jets.jet_velocity_km_s == 50.0
